...
public class NotificationController {
    ...
    @GetMapping("{userId}")
    public ApiResponse<Page<Notification>> getAllNotification(@PathVariable String userId,
                                                         @RequestParam(required = false, defaultValue = "1") int page,
                                                         @RequestParam(required = false, defaultValue = "5") int size) {
        return ApiResponse.successWithResult(notificationService.getAllNotification(userId, page, size));
    }
